...
const PayInterestModal = ({ isOpen, onClose, loanId, loanType, onSuccess }: TProps) => {
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        if (isInstallmentType) {
          ...
          currentPeriod = sortedPeriods.find((p: any) => 
            p.status !== 'paid'
          );
          ...
          const milestone30 = sortedPeriods.find((p: any) => p.milestone_day === 30) 
    ...
    <Modal isOpen={isOpen} onClose={handleClose} size="lg">
      <ModalContent>
        ...
        <ModalBody>
                ...
                <div className="p-3 rounded-lg bg-primary-50 dark:bg-primary-900/20 border border-primary-200 dark:border-primary-800">
                  <div className="flex items-start gap-2">
                    ...
                    <div className="text-sm text-primary-700 dark:text-primary-400">
                      <p className="font-semibold">Gói 1: Đóng tiền tổng chuộc theo mốc</p>
                      <p className="mt-1">Đóng tiền bao gồm Gốc + Lãi + Phí. Có thể đóng nhiều lần cho đến khi đủ mốc hiện tại.</p>
                    </div>
                  </div>
                </div>
              )}
                ...
                <div className="p-3 rounded-lg bg-warning-50 dark:bg-warning-900/20 border border-warning-200 dark:border-warning-800">
                  <div className="flex items-start gap-2">
                    ...
                    <div className="text-sm text-warning-700 dark:text-warning-400">
                      <p className="font-semibold">Gói 2/3: Đóng tiền theo mốc 30 ngày</p>
                      <p className="mt-1">Luôn đóng lãi + phí của mốc 30 ngày (bất kể đang ở mốc nào). Có thể đóng nhiều lần cho đến khi đủ. Đóng xong mốc 30 ngày rồi thì XONG, không cho đóng thêm nữa.</p>
                    </div>
                  </div>
                </div>
              )}
                ...
                <div className="p-3 rounded-lg bg-danger-50 dark:bg-danger-900/20 border border-danger-200 dark:border-danger-800">
                  <div className="flex items-start gap-2">
                    ...
                    <div className="text-sm text-danger-700 dark:text-danger-400">
                      <p className="font-semibold">Đã đóng xong mốc 30 ngày rồi</p>
                      <p className="mt-1">Gói 2/3 đã đóng xong mốc 30 ngày. Không cho đóng thêm nữa.</p>
                    </div>
                  </div>
                </div>
              )}
          ...
          <Input
            label={isInstallmentType ? "Số tiền đóng (Gốc + Lãi + Phí)" : "Số tiền đóng (Lãi + Phí)"}
            ...
            description={remainingInterest > 0 ? `Tối đa: ${formatCurrencyVND(remainingInterest)}` : "Đã đóng đủ"}
            isRequired
            isDisabled={isSubmitting || remainingInterest <= 0 || (isBulletPaymentType && hasExistingPayment)}
          />
