...
const isRpcNotFoundError = (error: { code?: string; message?: string } | null) => {
...
const revertLoanToPendingFallback = async (
  ...
  if (fetchError || !loan) {
    return { ok: false as const, status: 404, error: "Không tìm thấy khoản vay" };
  }
  ...
  if (updateError) {
    ...
    return { ok: false as const, status: 500, error: "Lỗi khi cập nhật trạng thái" };
  }
