...
export async function generatePDFFromHTML(html: string): Promise<Buffer> {
  try {
    ...
    console.log(
      `[PDF_GAS] Calling Google Apps Script at ${GAS_PDF_SERVICE_URL}`,
    );
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Google Apps Script error: ${error.error || response.statusText}`,
      );
    }
    ...
    console.log(
      `[PDF_GAS] PDF generated successfully, size: ${buffer.length} bytes`,
    );
...
export async function checkGASPDFServiceHealth(): Promise<boolean> {
  try {
    if (!GAS_PDF_SERVICE_URL) {
      console.error(
        "[PDF_GAS] PDF_SERVICE_URL environment variable is not set",
      );
